const CACHE = 'mc-casa-v3'; // arranque instantaneo (stale-while-revalidate + SDK precacheado)
...
self.addEventListener('activate', e => {
  ...
  e.waitUntil(
    caches.keys().then(keys =>
      Promise.all(keys.filter(k => k.startsWith('mc-casa-') && k !== CACHE).map(k => caches.delete(k)))
